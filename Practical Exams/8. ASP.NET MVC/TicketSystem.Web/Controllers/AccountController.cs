namespace TicketSystem.Web.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        {
            IdentityManager = new AuthenticationIdentityManager(new IdentityStore(new TicketDbContext()));
        }
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                var user = new Author { UserName = model.UserName, Points = 10 };
        ...
        public async Task<ActionResult> ExternalLoginConfirmation(ExternalLoginConfirmationViewModel model, string returnUrl)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                IdentityResult result = await IdentityManager.Authentication.CreateAndSignInExternalUserAsync(AuthenticationManager, new Author { UserName = model.UserName, Points = 10 });
                if (result.Success)
                {
                    return RedirectToLocal(returnUrl);
                }
