...
namespace GSMCharacteristics
{
    ...
    //display characteristics (size and number of colors). Define 3 
    //separate classes (class GSM holding instances of the classes Battery and Display).

    //2. Define several constructors for the defined classes 
    //that take different sets of arguments (the full information for 
    ...
    //5. Use properties to encapsulate the data fields 
    //inside the GSM, Battery and Display classes. Ensure all fields hold correct data at any given time.
    ...
    //9. Add a property CallHistory in the GSM class to hold a list 
    //of the performed calls. Try to use the system class List<Call>.
    ...
    //in the call history. Assume the price per minute is fixed and is provided as a parameter.
    ...
    class GSMTest
    {
        //!!!Uncomment this to test GSMTest and comment the Main Method in GSMCallHistoryTest.cs
        ...
        static void Main()
        {
            ...
            GSM nokiaN97Mini = new GSM("N97Mini", "Nokia", 199.00M, "Ifaka", liONBattery, bigDisplay);
            GSM samsungGalaxyS3 = new GSM("GalaxyS3", "Samsung", 399.99M);

            listOfGSMs.Add(nokiaN97Mini);
            listOfGSMs.Add(samsungGalaxyS3);

            for (int i = 0; i < listOfGSMs.Count; i++)
            {
                Console.WriteLine(listOfGSMs[i]);
            }
